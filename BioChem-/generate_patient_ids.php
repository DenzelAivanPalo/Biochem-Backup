...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['selected_employees'])) {
    ...
    $stmt = $conn->prepare("SELECT MAX(CAST(SUBSTRING(patient_id, 2) AS UNSIGNED)) as max_id FROM employees");
    $stmt->execute();
    ...
    foreach ($selectedEmployees as $index => $employeeId) {
        ...
        $stmt = $conn->prepare("SELECT e.*, c.name as company_name,
                              ...
                              LEFT JOIN employee_tests elt ON e.id = elt.employee_id
                              LEFT JOIN tests lt ON elt.test_id = lt.id
                              WHERE e.id = ?
                              GROUP BY e.id");
